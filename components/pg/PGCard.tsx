...
export function PGCard({
  ...
  const handleDelete = (e: React.MouseEvent) => {
    e.preventDefault();
    e.stopPropagation();
    if (onDelete && confirm("Delete this PG listing?")) onDelete(pg.id);
  };

  return (
    <div className="group relative overflow-hidden rounded-2xl border border-slate-100 bg-white shadow-sm transition-all duration-300 hover:shadow-md hover:border-slate-200">
      {isAdmin && onDelete && (
        <button
          type="button"
          onClick={handleDelete}
          className="absolute right-3 top-3 z-10 rounded-lg bg-red-600 px-3 py-1.5 text-xs font-medium text-white shadow hover:bg-red-700"
        >
          Delete
        </button>
      )}
      <Link href={`/pg/${pg.id}`} className="block">
        ...
        <div className="border-t border-slate-50 p-5 transition-colors group-hover:border-slate-100">
          ...
          <div className="mt-3 flex flex-wrap items-center gap-x-4 gap-y-1 text-sm text-slate-600">
            <span className="font-semibold text-slate-900">₹{pg.rent.toLocaleString()}</span>
            <span className="text-slate-300">·</span>
            <span>{pg.distance} km from campus</span>
          </div>
        </div>
      </Link>
    </div>
