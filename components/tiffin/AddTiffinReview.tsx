...
type AddTiffinReviewProps = {
  tiffinCenterId: string;
};

export default function AddTiffinReview({ tiffinCenterId }: AddTiffinReviewProps) {
  ...
  const submit = async (e: React.FormEvent) => {
    ...
    setError("");
    try {
      const res = await fetch("/api/tiffin-reviews", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ tiffinCenterId, rating, comment: comment.trim() }),
      });
    ...
    <div className="mt-8 border-t border-slate-100 pt-8">
      {!showForm ? (
        ...
        <form onSubmit={submit} className="rounded-2xl border border-slate-100 bg-slate-50/50 p-6">
          ...
          <label htmlFor="tiffin-review-comment" className="mt-4 block text-sm font-medium text-slate-700">Comment</label>
          <textarea
            id="tiffin-review-comment"
            value={comment}
